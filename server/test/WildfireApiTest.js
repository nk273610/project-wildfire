...
describe("/GET get the wildfire data API test", () => {
    ...
    it("It should send validation error for Wildfire API as incorrect query param is present", (done) => {
        chai.request(server)
            .get("/api/wildfires/ended?month=ma&year=2020")
            .end((err, res) => {
                res.should.have.status(400);
    ...
    it("It should send validation error for Wildfire API as incorrect query param is present", (done) => {
        chai.request(server)
            .get("/api/wildfires/ended?month=mar")
            .end((err, res) => {
                res.should.have.status(400);
    ...
    it("It should send validation error for Wildfire API as incorrect query param is present", (done) => {
        chai.request(server)
            .get("/api/wildfires/ended?month=mar&year=202")
            .end((err, res) => {
                res.should.have.status(400);
    ...
    it("It should send validation error for Wildfire API as incorrect query param is present", (done) => {
        chai.request(server)
            .get("/api/wildfires/ended?month=mar&year=20224")
            .end((err, res) => {
                res.should.have.status(400);
                res.body.should.have.property("message").eql("The year parameter is invalid");
                done();
